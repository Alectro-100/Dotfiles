...
function configure_vim_and_neovim() {
    ...
    if [[ ! -f "$HOME/.vimrc" ]]; then
        ...
        MESSAGE="There's already a vimrc config file do you want to back it up (enter/no)?"
        if user_input "$MESSAGE" ; then
...
read -r -p "Do you want to install the software packages? (y/n): " yn
if [[ $yn =~ ^[Yy]$ ]]; then
    install_software
elif [[ $yn == "" ]]; then
    install_software
...
read -r -p "Do you want to copy the configuration files? (y/n): " yn
if [[ $yn =~ ^[Yy]$ ]]; then
    copy_configs
elif [[ $yn == "" ]]; then
    copy_configs
...
read -r -p "Do you want to set up vim and neovim? (y/n)" yn
if [[ $yn =~ ^[Yy]$ ]]; then
